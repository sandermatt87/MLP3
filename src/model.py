...
class model:
	...
	def train(self,targets,nsplits,nclasses):
		...
		if(self.prediction_cache_exists):
			...
			self.predictor = OneVsRestClassifier(SVC(gamma=0.001/self.train_features.shape[1]))
			np.random.seed(1231)
			self.cv_predictions=np.copy(targets)*0
			self.cv_score=0
			kf = KFold(n_splits=nsplits)
			for train, test in kf.split(self.train_features):
				self.predictor.fit(self.train_features[train],new_targets[train])
				print self.predictor.classes_
				tmp=self.predictor.predict(self.train_features[test])
				self.cv_predictions[test]=utils.to_multiple_classes(tmp,nclasses)
				self.cv_score+=hamming_loss(targets[test],self.cv_predictions[test],labels=(0,1))
			self.cv_score/=nsplits
			print "cv_hamming_loss: "+str(self.cv_score)
		
			self.predictor.fit(self.train_features, new_targets)
	...
	def predict(self):
		...
		if(self.prediction_cache_exists):
			...
			if(self.train_features is None or self.train_features.shape[1]<2):
				self.cv_predictions=np.zeros(self.ntrain)
				self.predictions=np.zeros(self.ntest)
