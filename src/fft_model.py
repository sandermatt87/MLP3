...
class fft_model(model.model):
	...
	def read_features(self,path):
		...
		for i in range(0,self.ntrain):
			...
			array=parse.voxels_from_image(filename,smoothening_width=self.smoothening,stride=[3,3,3])
			tmp.append(preprocess.fft_features1D(array,self.ncubes))
