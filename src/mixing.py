...
def cv_optimization(cv_predictions,targets,nmodel):
	...
	def loss(weights):
		...
		norm_weights=np.insert(norm_weights, 0, 0.1) #fix the first entry (before normalisation) so that the solution is unique (and positive)
	...
	optimal_weights = np.insert(optimal_weights, 0, 0.1)
